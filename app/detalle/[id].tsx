...
export default function DetalleScreen() {
  ...
  const [editedName, setEditedName] = useState(creature?.name || '');
  const [editedType, setEditedType] = useState<CreatureType>(creature?.type || 'dragon');
  const [editedDescription, setEditedDescription] = useState(creature?.description || '');
  const [editedImageUrl, setEditedImageUrl] = useState(creature?.imageUrl || '');
  const [editedAbilities, setEditedAbilities] = useState(creature?.abilities.join(', ') || '');
  const [editedDangerLevel, setEditedDangerLevel] = useState<DangerLevel>(creature?.dangerLevel || 1);
  const [editedLastSeen, setEditedLastSeen] = useState(creature?.lastSeen || '');
